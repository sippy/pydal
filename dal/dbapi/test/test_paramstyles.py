...
class ParamstyleTests(unittest.TestCase):
    ...
    def testParamstyles(self):
        for test in self.tests:
            ...
            assert actual_query == expected_query, 'from=%s, to=%s, input_query=%s, input_params=%s, expected_query=%s, expected_params=%s, actual_query=%s, actual_params=%s' % (from_paramstyle, to_paramstyle, input_query, input_params, expected_query, expected_params, actual_query, actual_params)
